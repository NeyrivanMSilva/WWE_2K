...
const getPPVs = async (req, res) => {
  ...
  let result = Ppv.find(queryObject)
  const ppvs = await result
  res.status(StatusCodes.OK).json(ppvs)
...
const createPPV = async (req, res) => {
  ...
  const ppv = await Ppv.create({ ...req.body });

  res.status(StatusCodes.CREATED).json({ ppv })
}

const createSinglesPPVMatch = async (req, res) => {
